...
class self.dataHelper:

    def __init__(self, self.data=None, copy=False, deep=False, verbose=2):
        """
        Constructor.

        Parameters
        ----------
        self.data : pandas.self.dataFrame, optional
            self.dataFrame to operate on. The default is None.  If None is specified, the 
            self.data should be loaded in a separate function call, e.g., with "LoadAndInspectself.data."
        deep : bool, optional
        ...
        self.self.data = None
        if copy:
            self.self.data = self.data.copy(deep)
        else:
    ...
    def LoadAndInspectself.data(self, inputFile):
        """
        ...
        self.self.data = pd.read_csv(inputFile)
        ...
        PrintNotAvailableCounts(self.data)
    ...
    def PrintNotAvailableCounts(self):
        ...
        None.
        """
        notAvailableCounts, totalNotAvailable = GetNotAvailableCounts(self.data)
        print("\nMissing entry counts:")
        print(notAvailableCounts)
    
        if totalNotAvailable:
            lendres.Console.PrintWarning("Some self.data entries are missing.")
            print("Total missing:", totalNotAvailable)
        else:
            print("No entries are missing.")
    
    
    def GetNotAvailableCounts(self.data):
        """
    ...
    def DropRowsWhereself.dataNotAvailable(self.data, category, inPlace=False):
        """
    ...
    def DropAllRowsWhereself.dataNotAvailable(self, inPlace=False):
        """
    ...
    def GetRowsWithMissingEntries(self.data):
        """
    ...
    def DropMinAndMaxValues(self, category, criteria, method="fraction", inPlace=False):
        ...
        minAndMaxValues = GetMinAndMaxValues(self.data, category, criteria, method=method)
    ...
    def DisplayAllCategoriesValueCounts(self, useMarkDown=False):
        ...
        """
        # Find all the category types in the self.dataFrame and loop over them.
        DisplayCategoryCounts(self.data, self.data.dtypes[self.data.dtypes == 'category'].index, useMarkDown)
    ...
    def RemoveRowsWithLowValueCounts(self, column, criteria):
        ...
        for value in dropValues:
             RemoveRowByEntryValue(self.data, column, value, inPlace=True)
    ...
    def MergeNumericalself.dataByRange(self, column, labels, boundaries, replaceExisting=False):
        """
    ...
    def EncodeAllCategoricalColumns(self):
        ...
        return EncodeCategoricalColumns(self.data, allCategoricalColumns)
